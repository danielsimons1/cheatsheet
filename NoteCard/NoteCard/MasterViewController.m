...
- (void)insertNewObject:(id)sender {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"New Note Card" message:@"Enter a name for this Note Card" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)managedObject {
    if ([[segue identifier] isEqualToString:@"tabBarDetail"]) {
        ...
        DataAccess *dataAccess = [DataAccess sharedInstance];
        CSData *noteCardDocument = [dataAccess.fetchedResultsController objectAtIndexPath:indexPath];
        
        if (!noteCardDocument) {
            noteCardDocument = managedObject;
        }
        
        CSTabBarViewController *tabBarViewController = (CSTabBarViewController *)[[segue destinationViewController] topViewController];
        [tabBarViewController setDetailItem:noteCardDocument];
